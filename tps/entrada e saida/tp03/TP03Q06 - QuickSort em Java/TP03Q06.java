...
class TP03Q06{
    //codigo quicksortt para int
    public static void quickSort(int[] paises, int inicio, int fim){
        int i = inicio, j = fim;
        int pivo = paises[(fim+inicio)/2];
        while (i <= j) {
            while (paises[i] < pivo) i++;
            while (paises[j] > pivo) j--;
            if (i <= j) {
                swap(paises, i, j);
                i++;
                j--;
        ...
        if (inicio < j)  
            quickSort(paises, inicio, j);
        if (i < fim)  
            quickSort(paises, i, fim);
    }

    public static void swap(int[] paises, int i, int j) {
        int temp = paises[i];
        paises[i] = paises[j];
        paises[j] = temp;
    }
