...
class TP03Q05{
    // Codigo HeapSort para int
    public static void heapSort(int[] paises, int n){
        for(int i = (n - 1)/2; i >= 0; i--){
            criaHeap(paises, i, n - 1);
        }
        for(int i = n - 1; i >= 1; i--){
            int aux = paises[0];
            paises[0] = paises[i];
            paises[i] = aux;
            criaHeap(paises, 0, i - 1);
        }
    }

    public static void criaHeap(int paises[], int i, int n){
        int aux = paises[i];
        int j = i * 2 + 1;
        while(j <= n){
            if(j < n){
